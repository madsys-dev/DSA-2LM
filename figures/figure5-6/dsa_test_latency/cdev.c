...
void idxd_xa_pasid_remove(struct idxd_user_context *ctx)
{
	struct idxd_wq *wq = ctx->wq;
	void *ptr;
...
static unsigned long long dsa_portal_phy_address = 0L;
static unsigned long long get_dsa_portal_phy_address(void) {
	return dsa_portal_phy_address;
}
// EXPORT_SYMBOL(get_dsa_portal_phy_address);
...
static int idxd_cdev_mmap(struct file *filp, struct vm_area_struct *vma)
{
	...
	dsa_portal_phy_address = (base + idxd_get_wq_portal_full_offset(wq->id,
				IDXD_PORTAL_LIMITED));
	pfn = dsa_portal_phy_address >> PAGE_SHIFT;
